...
//      code here :- ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'Amit@123';
...
// I suggest always we press (ctrl+c).

const express = require("express")  // require express 
const app = express()
const knex = require("./cannetion/database.js")
const port = 6000

app.use(express.json())     // use express object form 
...
app.get('/which_user_data_find/:id', async (req,res)=>{
    try {
        const data = await knex('user_data').where({'id':req.params.id})
        res.send(data)
        console.log(data);
    } catch (error) {
        res.send(error.message)
...
// -----------------which user data update with id ? on postman .
...
app.delete("/delete/:id",async(req,res)=>{
    try{
        const data =await knex("user_data").where({'id':req.params.id}).del(req.body);
